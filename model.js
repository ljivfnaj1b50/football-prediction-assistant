...
function formStats(team = {}) {
  ...
  if (!rows.length) return { points: 1.35, gf: 1.25, ga: 1.25 };
  return {
    points: rows.reduce((s, m) => s + (m.result === "W" ? 3 : m.result === "D" ? 1 : 0), 0) / rows.length,
...
function teamFriction(team = {}, match = {}) {
  ...
  const rest = rd == null ? 0 : rd < 3 ? 0.12 : rd < 4 ? 0.06 : rd > 8 ? 0.03 : 0;
...
function expectedGoals(match) {
  ...
  home *= 1 - env * 0.42;
  away *= 1 - env * 0.42;
  return { home: clamp(home, 0.25, 3.35), away: clamp(away, 0.2, 3.2), env, homeDrag, awayDrag };
}
...
function scoreMatrix(homeXg, awayXg) {
  ...
  return {
    ...
    scores: scores.sort((a, b) => b.p - a.p).slice(0, 5).map(s => ({ score: s.score, p: s.p / mass })),
  };
}
...
export function analyzeMatch(match) {
  ...
  const missing =
    ...
    (match.home?.form?.length ? 0 : 0.1) +
    (match.away?.form?.length ? 0 : 0.1) +
  ...
  const blended = marketProb
    ...
    : model.prob;
  const pick = [
    ["home", blended.home, `${match.home?.name || match.homeTeam}方向`],
    ["draw", blended.draw, "平局防线"],
    ["away", blended.away, `${match.away?.name || match.awayTeam}方向`],
  ].sort((a, b) => b[1] - a[1])[0];
  ...
  const goals = model.totals.map((p, goals) => ({ goals: goals === 7 ? "7+" : goals, p })).sort((a, b) => b.p - a.p);
  ...
  const explanation = [];
  if (xg.homeDrag > 0.12) explanation.push(`${match.home?.name}伤停/停赛/疲劳折损偏高`);
  if (xg.awayDrag > 0.12) explanation.push(`${match.away?.name}伤停/停赛/疲劳折损偏高`);
  if (xg.env > 0.16) explanation.push("天气/海拔对比赛节奏有压制");
  if (market.heat > 0.14) explanation.push(`市场热度集中：${market.hotSide}`);
  if (!explanation.length) explanation.push("主要信号相对均衡，没有触发极端风险因子");
  return {
    id: match.id,
    match,
    xg: { home: round(xg.home, 2), away: round(xg.away, 2) },
    probabilities: { home: round(blended.home * 100), draw: round(blended.draw * 100), away: round(blended.away * 100) },
    ...
    direction: { key: pick[0], label: pick[2], p: round(pick[1] * 100) },
    goals: { main: goals.slice(0, 3).sort((a, b) => String(a.goals).localeCompare(String(b.goals))).map(x => x.goals), backup: goals.slice(3, 5).map(x => x.goals) },
    scores: model.scores.map(s => ({ score: s.score, p: round(s.p * 100) })),
    market,
    risk,
    riskScore: round(riskScore, 2),
    confidence: round((1 - riskScore) * 100),
