...
const OPENLIGADB_LEAGUES = (process.env.OPENLIGADB_LEAGUES || 'wm26:2026,bl1:2026,bl2:2026,bl3:2026').split(',').map(s => s.trim()).filter(Boolean);
const LIVE_TTL_MS = Number(process.env.LIVE_TTL_MS || 10 * 60 * 1000);

function send(res, code, data) {
  const body = JSON.stringify(data, null, 2);
  res.writeHead(code, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store', 'access-control-allow-origin': '*', 'access-control-allow-methods': 'GET,POST,OPTIONS', 'access-control-allow-headers': 'content-type,x-admin-token' });
  res.end(body);
}
function readBody(req) { return new Promise((resolve, reject) => { let body = ''; req.on('data', chunk => { body += chunk; if (body.length > 1024 * 1024 * 2) { reject(new Error('body too large')); req.destroy(); } }); req.on('end', () => resolve(body)); req.on('error', reject); }); }
...
function requireAdmin(req, res) { if (hasAdmin(req)) return true; send(res, 401, { ok: false, message: '权限校验失败' }); return false; }
function readJson(file, fallback = null) { try { return JSON.parse(fs.readFileSync(file, 'utf-8')); } catch { return fallback; } }
function writeJson(file, payload) { fs.mkdirSync(path.dirname(file), { recursive: true }); fs.writeFileSync(file, JSON.stringify(payload, null, 2), 'utf-8'); }
function validatePayload(payload) { if (!payload || typeof payload !== 'object') return '数据必须是对象'; if (!Array.isArray(payload.matches)) return 'matches 必须是数组'; for (const item of payload.matches) { if (!item.id) return '每场比赛必须有 id'; if (!item.home || !item.away) return `${item.id} 缺少 home/away`; if (!item.home.name || !item.away.name) return `${item.id} 缺少球队名称`; if (!item.kickoff) return `${item.id} 缺少 kickoff`; if (!item.venue) return `${item.id} 缺少 venue`; if (!item.weather) return `${item.id} 缺少 weather`; } return ''; }
function backupCurrentData() { if (!fs.existsSync(DATA_FILE)) return null; fs.mkdirSync(BACKUP_DIR, { recursive: true }); const stamp = new Date().toISOString().replace(/[:.]/g, '-'); const backupFile = path.join(BACKUP_DIR, `matches-${stamp}.json`); fs.copyFileSync(DATA_FILE, backupFile); return backupFile; }
function listBackups() { if (!fs.existsSync(BACKUP_DIR)) return []; return fs.readdirSync(BACKUP_DIR).filter(name => name.endsWith('.json')).sort().reverse().slice(0, 30).map(name => ({ name, file: path.join(BACKUP_DIR, name) })); }
...
function normalizeLogo(url) { return String(url || '').replace(/^http:/, 'https:'); }
function mapOpenLigaMatch(raw) {
  const r = Array.isArray(raw.matchResults) ? raw.matchResults.find(x => Number(x.resultTypeID) === 2) || raw.matchResults[raw.matchResults.length - 1] : null;
  const hGoals = Number(r?.pointsTeam1 ?? 0);
  const aGoals = Number(r?.pointsTeam2 ?? 0);
  const isFinished = Boolean(raw.matchIsFinished);
  const formResult = isFinished ? resultToForm(hGoals, aGoals) : null;
  return {
    id: `openliga-${raw.matchID}`,
    sourceId: raw.matchID,
    competition: raw.leagueName || 'OpenLigaDB',
    stage: raw.group?.groupName || raw.groupName || '赛程',
    kickoff: raw.matchDateTimeUTC || raw.matchDateTime || new Date().toISOString(),
    status: isFinished ? '已完赛' : '未开赛',
    neutral: false,
    venue: { name: raw.location?.locationStadium || '待确认球场', city: raw.location?.locationCity || '', altitudeM: 0 },
    home: { ...defaultTeam(raw.team1?.teamName || '主队', normalizeLogo(raw.team1?.teamIconUrl), ''), form: formResult ? [{ result: formResult.home, gf: hGoals, ga: aGoals }] : [] },
    away: { ...defaultTeam(raw.team2?.teamName || '客队', normalizeLogo(raw.team2?.teamIconUrl), ''), form: formResult ? [{ result: formResult.away, gf: aGoals, ga: hGoals }] : [] },
...
function enrichForms(matches) {
  const history = new Map();
  for (const m of [...matches].sort((a, b) => new Date(a.kickoff) - new Date(b.kickoff))) {
    const hKey = m.home.name;
    const aKey = m.away.name;
    m.home.form = (history.get(hKey) || []).slice(-5);
    m.away.form = (history.get(aKey) || []).slice(-5);
    if (m.status === '已完赛') {
      const h = m.home.form[0];
      const a = m.away.form[0];
      if (h) history.set(hKey, [...(history.get(hKey) || []), h].slice(-5));
      if (a) history.set(aKey, [...(history.get(aKey) || []), a].slice(-5));
    }
  }
  return matches;
}
...
async function loadOpenLigaDb() {
  ...
  for (const item of OPENLIGADB_LEAGUES) {
    const [league, season] = item.split(':');
    if (!league || !season) continue;
    try {
      const rows = await fetchUrlJson(`https://api.openligadb.de/getmatchdata/${encodeURIComponent(league)}/${encodeURIComponent(season)}`);
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && pathname === '/api/matches') { if (!requireAdmin(req, res)) return; try { const body = await readBody(req); const payload = JSON.parse(body); const err = validatePayload(payload); if (err) return send(res, 400, { ok: false, message: err }); const backupFile = backupCurrentData(); payload.updatedAt = new Date().toISOString(); payload.mode = payload.mode || 'internal-data-file'; writeJson(DATA_FILE, payload); writeJson(PUBLIC_DATA_FILE, payload); return send(res, 200, { ok: true, message: '保存成功，前台数据已同步，旧数据已备份', updatedAt: payload.updatedAt, count: payload.matches.length, backupFile, dataFile: DATA_FILE, publicDataFile: PUBLIC_DATA_FILE }); } catch (err) { return send(res, 500, { ok: false, message: '保存失败', detail: err.message }); } }
  return send(res, 404, { ok: false, message: 'not found' });
});
server.listen(PORT, '127.0.0.1', () => { console.log(`jingxi-football-api running on 127.0.0.1:${PORT}`); setTimeout(() => loadPublicFeed(true).catch(err => console.error('auto refresh failed', err.message)), 3000); setInterval(() => loadPublicFeed(true).catch(err => console.error('auto refresh failed', err.message)), LIVE_TTL_MS); });
